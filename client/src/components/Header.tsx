...
export default function Header({ onOpenShop }: HeaderProps) {
    ...
    const getBadgeIcon = () => {
        ...
        if (previewTheme?.badgeUrl) {
            ...
            return <img src={previewTheme.badgeUrl} alt="Preview Badge" className={`object-contain ${isLocal ? 'w-10 h-10 scale-[2]' : 'w-4 h-4'}`} />;
        }
        ...
        if (equippedBadge && BADGE_URLS[equippedBadge]) {
            ...
            return <img src={badgeUrl} alt="Badge" className={`object-contain ${isLocal ? 'w-10 h-10 scale-[2]' : 'w-4 h-4'}`} />;
        }
