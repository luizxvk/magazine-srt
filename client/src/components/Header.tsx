...
export default function Header({ onOpenShop }: HeaderProps) {
    ...
    const getBadgeIcon = () => {
        ...
        if (previewTheme?.badgeUrl) {
            ...
            return <img src={previewTheme.badgeUrl} alt="Preview Badge" className={`object-contain ${isLocal ? 'w-5 h-5' : 'w-4 h-4'}`} />;
        }
        ...
        if (equippedBadge && BADGE_URLS[equippedBadge]) {
            ...
            return <img src={badgeUrl} alt="Badge" className={`object-contain ${isLocal ? 'w-5 h-5' : 'w-4 h-4'}`} />;
        }
