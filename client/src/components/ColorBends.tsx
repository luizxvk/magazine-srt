...
void main() {
  ...
  if (uColorCount > 0) {
    ...
    for (int i = 0; i < MAX_COLORS; ++i) {
      ...
      s = abs(s) - 0.5;
      s *= 1.0 + uWarpStrength * 0.3;
      s -= uFrequency * 0.5 * cos(t * 0.13 + float(i)) * vec2(0.3, 0.2);
      float d = dot(s, s);
      vec3 bandColor = uColors[i];
      float intensity = exp(-2.5 * d) * (0.7 + 0.5 * sin(t * 0.3 + float(i)));
      sumCol += bandColor * intensity;
      cover += intensity;
    }
    ...
    for (int k = 0; k < 3; ++k) {
      s = abs(s) - 0.5;
      s *= 1.0 + uWarpStrength * 0.3;
      s -= uFrequency * 0.5 * cos(t * 0.13 + float(k)) * vec2(0.3, 0.2);
      float d = dot(s, s);
      float wave = sin(d * 6.28 + t) * 0.5 + 0.5;
      col[k] = wave * exp(-1.5 * d);
    }
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: frag,
      uniforms: {
        uCanvas: { value: new THREE.Vector2(container.clientWidth || 1, container.clientHeight || 1) },
        uTime: { value: 0 },
        uSpeed: { value: speed },
