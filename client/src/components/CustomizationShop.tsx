...
const featuredBackgrounds: Omit<ShopItem, 'owned' | 'equipped'>[] = [
    { id: 'anim-cosmic-triangles', name: 'Tringulos Cósmicos', description: 'Tringulos 3D coloridos em movimento hipnótico', price: 2500, type: 'background', preview: 'radial-gradient(circle at 30% 30%, #ff0080 0%, transparent 30%), radial-gradient(circle at 70% 70%, #00ffff 0%, transparent 30%), radial-gradient(circle at center, #111 0%, #000 100%)' },
    ...
    { id: 'anim-infinite-triangles', name: 'Infinite Triangles', description: 'Grade hexagonal com tringulos infinitos na cor de destaque', price: 3500, type: 'background', preview: 'linear-gradient(135deg, var(--accent-color, #d4af37) 0%, rgba(212,175,55,0.3) 50%, #000 100%)' },
    ...
    { id: 'anim-dark-veil', name: 'Véu Sombrio', description: 'Rede neural CPPN gerando padrões orgnicos hipnóticos', price: 5000, type: 'background', preview: 'radial-gradient(ellipse at center, #2a0845 0%, #1a0530 30%, #0a0115 100%)' },
...
const colors: Omit<ShopItem, 'owned' | 'equipped'>[] = [
    defaultItems.color,
    { id: 'color_rgb', name: 'RGB Dinmico', description: 'Troca entre Red, Green, Blue ao vivo!', price: 1000, type: 'color', preview: 'rgb-dynamic' },
    ...
    { id: 'color_pastel_lilac', name: 'Lilás Pastel', description: 'Romntico e místico', price: 350, type: 'color', preview: '#dda0dd' },
    ...
    const getItems = () => {
        ...
        switch (activeTab) {
            ...
            case 'color': {
                ...
                const basicColors = allColors.filter(c => !c.id.includes('pastel') && !c.id.includes('gradient'));
                const pastelColors = allColors.filter(c => c.id.includes('pastel'));
                const gradientColors = allColors.filter(c => c.id.includes('gradient'));
                return { basicColors, pastelColors, gradientColors };
            }
            case 'profileBorder': {
                ...
                >
                    ...
                    <div className="p-4 overflow-y-auto max-h-[calc(85vh-180px)] custom-scrollbar">
                        {activeTab === 'packs' ? (
                            ...
                            (() => {
                                const { basicColors, pastelColors, gradientColors } = getItems() as { basicColors: typeof colors; pastelColors: typeof colors; gradientColors: typeof colors };
                                return (
                                    <>
                                        {/* Basic Colors */}
                                        <div className="mb-6">
