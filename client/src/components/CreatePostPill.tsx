...
export default function CreatePostPill({ onClick }: CreatePostPillProps) {
    ...
    return (
        <>
            ...
            >
                {/* Desktop: Plus que gira */}
                <Plus className="w-5 h-5 md:w-5 md:h-5 transition-transform duration-300" />
                
                {/* Mobile: Seta que aparece quando não expandido */}
                {!isExpanded && (
                    <ChevronUp className="w-4 h-4 md:hidden animate-bounce" />
                )}
